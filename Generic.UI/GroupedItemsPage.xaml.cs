using Generic.UI.Common;
using Generic.UI.Flyouts;
...
namespace Generic.UI
{
    ...
    /// </summary>
    public sealed partial class GroupedItemsPage : Generic.UI.Common.LayoutAwarePage
    {
        ...
        protected override void LoadState(Object navigationParameter, Dictionary<String, Object> pageState)
        {
            ...
            {
                App.ShowSimpleMessage("Sorry, an error has ocurred, please try restarting the application.", "Starting app");
            }
        }
        ...
        void ItemView_ItemClick(object sender, ItemClickEventArgs e)
        {
            // Navigate to the appropriate destination page, configuring the new page
            // by passing required information as a navigation parameter
            ParentViewModel item = ((ParentViewModel)e.ClickedItem);
        ...
        private void itemListView_SelectionChanged(object sender, Windows.UI.Xaml.Controls.SelectionChangedEventArgs e)
        {
            ...
            {
                if (((ParentViewModel)item).Identifier == -1)
                {
            ...
            foreach (object item in e.AddedItems)
            {
                ...
                ((ParentViewModel)item).IsSelected = true;
            }

            foreach (object item in e.RemovedItems)
            {
                itemGridView.SelectedItems.Remove(item);
                ((ParentViewModel)item).IsSelected = false;
            }
        ...
        async internal void NavigateToLaunchedFromSecondaryTile()
        {
            ...
            ParentViewModel item = await MainViewModel.GetTrip(identifier);
            MainViewModel.SetSelectedTrip(item);
            this.Frame.Navigate(typeof(ItemDetailPage), item.Identifier);
        }
    }
